...
{
    std::optional locationCheck = DeviceManager::getInstance().getCurrentWorkspace();
    if (!locationCheck) return;
    std::string location = locationCheck.value() + "/StatusBar/HomeDomain/Library/SpringBoard/statusBarOverrides";
    qDebug() << location;
